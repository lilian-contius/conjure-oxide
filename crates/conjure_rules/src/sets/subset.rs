...
fn subset_to_subsetEq_neq(expr: &Expression, _: &SymbolTable) -> ApplicationResult {
    match expr {
        Subset(_, a1, a2) => match (a1.as_ref(), a2.as_ref()) {
            Atomic(m1, Atom::Reference(a1)) => {
                let expr1 = Expression::Atomic(m1.clone(), Atom::Reference(a1.clone()));
                match rhs.as_ref() {
                    Atomic(m2, Atom::Reference(a2)) => {
                        let expr2 = Expression::AbstractLiteral(m2.clone(), a2.clone());
                        let expr3 = SubsetEq(
                            Metadata::new(),
                            Box::new(expr1.clone()),
                            Box::new(expr2.clone()),
                        ...
                        let expr5 = Not(Metadata::new(), Box::new(expr4.clone()));

                        Ok(Reduction::pure(And(
                            Metadata::new(),
                            Box::new(matrix_expr![expr3.clone(), expr5.clone()]),
                        )))
                    }
